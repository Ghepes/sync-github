  ...
  function initEventListeners(config) {
    ...
    if (config.showSuggestions) {
      const fetchSuggestions = debounce(async (query) => {
        ...
        try {
              // Instead of directly calling your API
              const API_URL = "https://search.wromo.com/api";

             // Use a CORS proxy
              const CORS_PROXY = "https://xmin.wromo.net/?";
              const response = await fetch(`${CORS_PROXY}${encodeURIComponent(`${API_URL}/suggestions?q=${query}`)}`);
