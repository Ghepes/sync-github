...
;(() => {
  ...
  function injectStyles(config) {
    ...
    style.textContent = `
      ...
      .satellite-frame-error h3 {
        margin-top: 0;
        color: #333;
      }
      
      .satellite-frame-error p {
        margin-bottom: 15px;
        color: #666;
      }
      
      .satellite-frame-error-button {
  ...
  function createSearchUI(container, config) {
    ...
    if (config.displayMode === "embed") {
      ...
      // Create a container for frame errors
      const frameErrorContainer = document.createElement("div")
      frameErrorContainer.className = "satellite-frame-error"
      frameErrorContainer.id = "satellite-frame-error"
      frameErrorContainer.style.display = "none"

      modalContent.appendChild(closeButton)
      modalContent.appendChild(iframe)
      modalContent.appendChild(frameErrorContainer)
  ...
  function initEventListeners(config) {
    ...
    const frameErrorContainer = document.getElementById("satellite-frame-error")
    ...
    function handleCardClick(result) {
      if (config.displayMode === "redirect") {
        ...
        embedContainer.onerror = () => {
          handleFrameError(result)
        }

        // Check if the iframe loaded correctly after a short delay
        setTimeout(() => {
          try {
            // Try to access iframe content - this will throw an error if X-Frame-Options blocks it
            const iframeContent = embedContainer.contentWindow.document
            // If we get here, the iframe loaded successfully
          } catch (error) {
            // If we can't access the iframe content, show the error message
            handleFrameError(result)
          }
        }, 1000)
      }
    ...
    function handleFrameError(result) {
      if (frameErrorContainer) {
        // Hide the iframe
        ...
        frameErrorContainer.innerHTML = `
          <h3>This website cannot be displayed in an iframe</h3>
          <p>The website you're trying to view doesn't allow embedding in other websites.</p>
          <a href="${result.link}" target="_blank" class="satellite-frame-error-button">Open in New Tab</a>
        `
      } else {
        // If there's no error container, just open in a new tab
        window.open(result.link, "_blank")
      }
    }
